...
def _classify_bysub_subject(
    ...
    """Classify rare vs frequent for one subject across all time points.
    ...
    Returns
    -------
    dict with metric arrays for this subject.
    """
    # Moving average
    ...
    for itime in range(nSamples):
        print(
            f"  {classifier_name} | sub {isub + 1} (ID={iid}) | "
        ...
        for train_ix, test_ix in kfold.split(X, y):
            ...
            meanSamples = int(np.mean([class_counts.get(0, 0), class_counts.get(1, 0)]))
            if meanSamples == 0:
                continue
            ...
            try:
                ...
                auc_std_list.append(roc_auc_score(y_test_orig, y_pred))
            except ValueError:
                pass
        ...
        acc_std_sub[itime] = np.std(acc_std_list) if acc_std_list else 0.0
        accbal_sub[itime] = accbal_kfold / nFolds
        accbal_std_sub[itime] = np.std(accbal_std_list) if accbal_std_list else 0.0
        auc_sub[itime] = auc_kfold / nFolds
        auc_std_sub[itime] = np.std(auc_std_list) if auc_std_list else 0.0
    ...
    return {
        ...
        "cm_sub": cm_sub,
        "acc_sub": acc_sub,
        "acc_std_sub": acc_std_sub,
        "accbal_sub": accbal_sub,
        "accbal_std_sub": accbal_std_sub,
        "auc_sub": auc_sub,
        "auc_std_sub": auc_std_sub,
        "precision_sub": precision_sub,
        "recall_sub": recall_sub,
        "f1_sub": f1_sub,
        "importance_acc_sub": importance_acc_sub,
        "importance_acc_std_sub": importance_acc_std_sub,
        "importance_accbal_sub": importance_accbal_sub,
        "importance_accbal_std_sub": importance_accbal_std_sub,
        "importance_auc_sub": importance_auc_sub,
        "importance_auc_std_sub": importance_auc_std_sub,
        "importance_mi_sub": importance_mi_sub,
    }
...
def classify_bysub(
    ...
    # Pre-extract per-subject data slices
    sub_data_list = []
    sub_labels_list = []
    for iid in unique_ids:
        ...
        sub_data_list.append(data_sub)
        sub_labels_list.append(labels[sub_idx])

    # Process subjects in parallel or sequentially
    effective_n_jobs = n_jobs if n_jobs != 1 and nSubs > 1 else 1

    if effective_n_jobs != 1:
        print(
            f"  {classifier_name} | bysub | processing {nSubs} subjects "
            f"in parallel (n_jobs={effective_n_jobs})"
        )
        sub_results = Parallel(n_jobs=effective_n_jobs, verbose=10)(
            delayed(_classify_bysub_subject)(
                isub,
                iid,
                sub_data_list[isub],
                classifier,
    ...
    # Initialize metric arrays and collect results
    ...
    for isub, r in enumerate(sub_results):
        probs_total[:, :, isub] = r["probs_sub"]
        cm_total[:, :, isub] = r["cm_sub"]
        acc_total[:, isub] = r["acc_sub"]
        acc_std_total[:, isub] = r["acc_std_sub"]
        accbal_total[:, isub] = r["accbal_sub"]
        accbal_std_total[:, isub] = r["accbal_std_sub"]
        auc_total[:, isub] = r["auc_sub"]
        auc_std_total[:, isub] = r["auc_std_sub"]
        precision_total[:, isub] = r["precision_sub"]
        recall_total[:, isub] = r["recall_sub"]
        f1_total[:, isub] = r["f1_sub"]
        importance_acc_total[:, :, isub] = r["importance_acc_sub"]
        importance_acc_std_total[:, :, isub] = r["importance_acc_std_sub"]
        importance_accbal_total[:, :, isub] = r["importance_accbal_sub"]
        importance_accbal_std_total[:, :, isub] = r["importance_accbal_std_sub"]
        importance_auc_total[:, :, isub] = r["importance_auc_sub"]
        importance_auc_std_total[:, :, isub] = r["importance_auc_std_sub"]
        importance_mi_total[:, :, isub] = r["importance_mi_sub"]
