...
for p = 1:numel(subjectDirs)
    ...
    initialVar_ASR = zeros(1, numFiles);
    denoisedVar_ASR = zeros(1, numFiles);
    time_ASR = zeros(1, numFiles);
    ...
    parfor f = 1:numFiles
        ...
        initialVar_ASR(f) = mean(var(EEG.data, [], 1));

        %% ASR Denoising (with bad channel rejection)
        tic
        ASR_burst_criterion=20;
        ASRtemp = pop_clean_rawdata(EEG, 'FlatlineCriterion',5, ...
            'ChannelCriterion',0.8,'LineNoiseCriterion',4, ...
            'Highpass','off','BurstCriterion',ASR_burst_criterion, ...
            'WindowCriterion','off','BurstRejection','off', ...
            'Distance','Euclidian');
        EEG  = pop_interp(ASRtemp,  EEG.chanlocs, 'spherical');
        time_ASR(f) = toc;
        
        % Average variance across columns
        denoisedVar_ASR(f) = mean(var(EEG.data));  
        ...
        %% Saving
        new_filename = [subjectEEGFileNames{f}(1:end-4) '_ASR'];
...
ALLEEG = [ALLEEG{:}];
save("ASR_preproc.mat", "ALLEEG", "-v7.3");
