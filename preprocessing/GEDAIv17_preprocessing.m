% AMBER Dataset GEDAIv1.7 Denoising
...
for p = 1:numel(subjectDirs)
    ...
    parfor f = 1:numFiles
        ...
        initialVar_GEDAI(f) = mean(var(EEG.data, [], 1));  

        %% GEDAI Denoising
        % (Constants moved inside or passed as broadcast)
        tic
        EEG = GEDAI(EEG); 
        time_GEDAI_plugin(f) = toc;

        % Store denoised variance
        denoisedVar_GEDAI(f) = mean(var(EEG.data, [], 1));  
        ...
        %% Saving
        new_filename = [subjectEEGFileNames{f}(1:end-4) '_GEDAI'];
...
ALLEEG = [ALLEEG{:}];
save("GEDAI_preproc.mat", "ALLEEG", "-v7.3");
