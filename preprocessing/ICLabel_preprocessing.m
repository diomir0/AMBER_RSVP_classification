% AMBER Dataset ICLabel Denoising
...
for p = 1:numel(subjectDirs)
    ...
    initialVar_ICLabel = zeros(1, numFiles);
    denoisedVar_ICLabel = zeros(1, numFiles);
    time_ICLabel = zeros(1, numFiles);
    ...
    parfor f = 1:numFiles
        ...
        initialVar_ICLabel(f) = mean(var(EEG.data, [], 1));

        % Bad channel rejection step prior to ICA (using ASR)
        orginal_channel_locations = EEG.chanlocs;
        ...
        % Use IClabel to keep non-artifactual components automatically 
        EEG= pop_iclabel(EEG, 'default');
        [maxprob, IC_class] = max(EEG.etc.ic_classification.ICLabel.classifications');
           
        keep_IC_brain = find(IC_class==1); % Brain ICs
        keep_IC_other = find(IC_class==7); % Other ICs
        
        try
            % Keep brain + other ICs
            EEG = pop_subcomp(EEG, [keep_IC_brain keep_IC_other], 0, 1); 
        catch 
        end
        ...
        EEG = pop_interp(EEG,  orginal_channel_locations, 'spherical'); 
        EEG = eeg_checkset(EEG);
        
        % Average variance across columns
        denoisedVar_ICLabel(f) = mean(var(EEG.data));  
       ...
       %% Epoching
        ...
        %% Saving
        new_filename = [subjectEEGFileNames{f}(1:end-4) '_ICLabel'];
...
% If you need ALLEEG back as a structure array after the loop:
ALLEEG = [ALLEEG{:}]; 
save("ICLabel_preproc.mat", "ALLEEG", "-v7.3");